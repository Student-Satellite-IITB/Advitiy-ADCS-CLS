...
v_w_IO_o=np.zeros((120000,2))

for i in range(0,120000):
	v_pos_magnitude[i] = np.linalg.norm(sgp_data[i,1:4]) #getting the value of distance of satellite from center earth
	v_w_IO_o[i,0] = np.sqrt(G*M_EARTH/(v_pos_magnitude[i])**3) #angular velocity of orbit frame wrt inertial frame in orbit frame

ang_vel_avg = np.average(v_w_IO_o[:,0])

for i in range(0,120000):
	v_w_IO_o[i,1] = (v_w_IO_o[i,0]-ang_vel_avg)/ang_vel_avg

'''
plt.plot(sgp_data[0:120000,0],v_pos_magnitude)
plt.xlabel('time' )
plt.ylabel('distance of satellite from center of earth')
plt.title('distance of satellite from center of earth versus time')
plt.show()

plt.plot(sgp_data[0:120000,0],v_w_IO_o)
plt.xlabel('time' )
plt.ylabel('angular velocity of orbit frame wrt inertial frame in orbit frame (v_w_IO_o)')
plt.title('v_w_IO_o versus time')
plt.show()
'''

plt.plot(sgp_data[0:120000,0],v_w_IO_o[:,1])
plt.xlabel('time' )
plt.ylabel('fractional error in angular velocity (v_w_IO_o)')
plt.title('fractional error in v_w_IO_o versus time')
plt.show()