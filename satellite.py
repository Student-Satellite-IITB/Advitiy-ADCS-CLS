...
class Satellite:
    ...
    def getW_BI_b(
            self):  # get exact angular velocity of body frame with respect to inertial frame expressed in body frame
        v_w_BO_b = self.v_state[4:7]
        v_q_BO = self.v_state[0:4]
        v_w_BI_b = fs.wBOb2wBIb(v_w_BO_b, v_q_BO,
                                v_w_IO_o)  # calculation of wBIb from wBOb stored in frames using wBOb2wBIb (function of frames) (v_w_IO_o from constants_1U)
        return v_w_BI_b
    ...
    def setggDisturbance_b(self,
                           v_gg_torque_dist_b):  # set torque due to gravity gradient about centre of mass in body frame
